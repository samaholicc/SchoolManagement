...
namespace SchoolManagement
{
                ...
                using (MySqlConnection conn = new MySqlConnection("Server=localhost;Database=system;User ID=root;Password=samia;"))
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            cbSubject.Items.Add($"_Lingobit_{subjectId} - {SubjectName}");
                        }
                    }
                }
            }
    ...
    try
    {
        using (MySqlConnection conn = new MySqlConnection("Server=localhost;Database=system;User ID=root;Password=samia;"))
        {
            ...
            {
                MessageBox.Show("_Lingobit_Invalid number of students. Please enter a valid number.");
                return;
            }
...
private void ClearInputs()
{
    ...
    dtpFinish.Value = DateTime.Now;
        ...
        private void ExportToExcel()
        {
            ...
            {
                MessageBox.Show("_Lingobit_Error exporting to Excel: " + ex.Message);
            }
            LoadClasses();
        }
