...
namespace SchoolManagement
{
    public partial class TeacherManager : KryptonForm
    {
        ...
        private void SaveTeacher()
        {
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    if (action == 0) // Ajout d'un nouvel enseignant
                    {
                        ...
                        using (var cmd = new MySqlCommand(query, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("_Lingobit_@gender", rbMale.Checked ? "Homme" : "Femme");
                    ...
                    else // Mise à jour d'un enseignant
                    {
                        ...
                        if (!string.IsNullOrEmpty(newPassword) && newPassword != currentPassword)
                        {
                            ...
                            {
                                MessageBox.Show("_Lingobit_Password updated successfully.");
                            }
                        }
                        else
                        {
            ...
            {
                MessageBox.Show("_Lingobit_Error: " + ex.Message);
            }
            finally
            {
                ClearInputs();
        ...
        private void dgvTeachers_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                MessageBox.Show($"Selected Teacher ID: {teacherId}"); // Affiche un message avec l'ID récupéré
        ...
        private int GetTotalRecordCount()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = "_Lingobit_SELECT COUNT(teacher_id) FROM SYSTEM.teacher";
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        object result = cmd.ExecuteScalar();
